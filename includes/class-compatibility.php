...
if ( ! class_exists( 'Show_Hide_TinyMCE_Compatibility' ) ) {

	final class Show_Hide_TinyMCE_Compatibility {
		...
		public function load_deprecated() {
			$load_deprecated = apply_filters( 'Show_Hide_tinymce_load_deprecated', true );
			if ( $load_deprecated ) {
		...
		public function load_plugins() {
			$load_compatibility_plugins = apply_filters( 'Show_Hide_tinymce_load_compatibility_plugins', array( 'siteorigin_panels', 'wpml', 'jetpack_after_the_deadline', 'wp_page_widget' ) );
			if ( ! empty( $load_compatibility_plugins ) ) {
				...
				self::$plugins = Show_Hide_TinyMCE_Compatibility_Plugins::instance( $load_compatibility_plugins );
			}
		}
		...
		public function load_wordpress() {
			if ( version_compare( get_bloginfo( 'version' ), '3.9', '<' ) ) {
				...
				self::$wordpress = Show_Hide_TinyMCE_Compatibility_Wordpress::instance();
			}
		}

	} // END class Show_Hide_TinyMCE_Compatibility
