...
if ( ! class_exists( 'Show_Hide_TinyMCE_Compatibility_Plugins' ) ) {

	final class Show_Hide_TinyMCE_Compatibility_Plugins {
		...
		public function wpml() {
			...
			add_action( 'Show_Hide_tinymce_before_widget', array( $this, 'wpml_widget_before' ), 10, 2 );
			add_action( 'Show_Hide_tinymce_after_widget', array( $this, 'wpml_widget_after' ), 10, 2 );
			add_filter( 'Show_Hide_tinymce_widget_update', array( $this, 'wpml_widget_update' ), 10, 2 );
		...
		public function wpml_init() {
			if ( is_plugin_active( 'sitepress-multilingual-cms/sitepress.php' ) && is_plugin_active( 'wpml-widgets/wpml-widgets.php' ) ) {
				if ( false !== has_action( 'update_option_widget_show-hide-tinymce', 'icl_st_update_widget_title_actions' ) ) {
					remove_action( 'update_option_widget_show-hide-tinymce', 'icl_st_update_widget_title_actions', 5 );
				}
			}
		}
		...
		public function wp_page_widget_admin_init() {
			if ( is_admin() && is_plugin_active( 'wp-page-widget/wp-page-widgets.php' ) && version_compare( get_bloginfo( 'version' ), '3.3', '>=' ) ) {
				add_filter( 'Show_Hide_tinymce_enable_pages', array( $this, 'wp_page_widget_enable_pages' ) );
		...
		public function wp_page_widget_enqueue_script() {
			$main_script = apply_filters( 'show-hide-tinymce-widget-script', 'show-hide-tinymce-widget' );
		...
		public function siteorigin_panels_admin_init() {
			if ( is_admin() && is_plugin_active( 'siteorigin-panels/siteorigin-panels.php' ) ) {
				...
				add_filter( 'Show_Hide_tinymce_container_selectors', array( $this, 'siteorigin_panels_container_selectors' ) );
				add_filter( 'Show_Hide_tinymce_activate_events', array( $this, 'siteorigin_panels_activate_events' ) );
				add_filter( 'Show_Hide_tinymce_deactivate_events', array( $this, 'siteorigin_panels_deactivate_events' ) );
				add_filter( 'Show_Hide_tinymce_enable_pages', array( $this, 'siteorigin_panels_enable_pages' ) );
		...
		public function siteorigin_panels_widget_object( $the_widget ) {
			if ( isset( $the_widget->id_base ) && 'show-hide-tinymce' == $the_widget->id_base ) {
				$the_widget->number = '';
			}
			return $the_widget;
		}
		...
		public function siteorigin_panels_disable_compat( ) {
			remove_action( 'admin_init', 'siteorigin_panels_Show_Hide_tinymce_admin_init' );
			remove_action( 'admin_enqueue_scripts', 'siteorigin_panels_Show_Hide_tinymce_admin_enqueue', 15 );
		}
		...
		public function jetpack_after_the_deadline() {
			add_action( 'Show_Hide_tinymce_load', array( $this, 'jetpack_after_the_deadline_load' ) );
		}
	...
	} // END class Show_Hide_TinyMCE_Compatibility_Plugins
