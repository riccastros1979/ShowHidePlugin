...
if ( ! class_exists( 'Show_Hide_TinyMCE_Admin' ) ) {

	final class Show_Hide_TinyMCE_Admin {
		...
		public function load_textdomain() {
			load_plugin_textdomain( 'show-hide-tinymce-widget', false, dirname( dirname( plugin_basename( __FILE__ ) ) ) . '/languages/' );
		}
		...
		public function enabled() {
			global $pagenow;
			$enabled_pages = apply_filters( 'Show_Hide_tinymce_enable_pages', array( 'widgets.php', 'customize.php', 'admin-ajax.php' ) );
			return apply_filters( 'Show_Hide_tinymce_enable', in_array( $pagenow, $enabled_pages ) );
		}
...
//				add_action( 'Show_Hide_tinymce_before_editor', array( $this, 'display_links' ) ); // consider donating if you remove links
				add_action( 'Show_Hide_tinymce_editor', array( $this, 'editor' ), 10, 4 );
				add_action( 'Show_Hide_tinymce_after_editor', array( $this, 'fix_the_editor_content_filter' ) );
				...
				do_action( 'Show_Hide_tinymce_load' );
			}
		}
		 ...
		 * @uses Show_Hide_TinyMCE_Plugin::enqueue_style()
		 *
		 * @return void
		 * @since 2.0.0
		 */
		...
		public function enqueue_style() {
			$style = apply_filters( 'show-hide-tinymce-widget-style', 'show-hide-tinymce-widget' );
		...
		public function enqueue_script() {
			$script = apply_filters( 'show-hide-tinymce-widget-script', 'show-hide-tinymce-widget' );
			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
			wp_enqueue_script(
				$script,
		...
		public function localize_script() {
			$container_selectors = apply_filters( 'Show_Hide_tinymce_container_selectors', array(  'div.widget', 'div.widget-inside' ) );
			$activate_events = apply_filters( 'Show_Hide_tinymce_activate_events', array() );
			$deactivate_events = apply_filters( 'Show_Hide_tinymce_deactivate_events', array() );
			$data = array(
				...
				'error_duplicate_id' => __( 'ERROR: Duplicate widget ID detected. To avoid content loss, please create a new widget with the same content and then delete this one.', 'show-hide-tinymce-widget' )
			);
			wp_localize_script( apply_filters( 'show-hide-tinymce-widget-script', 'show-hide-tinymce-widget' ), 'bstw_data', $data );
		}
		...
		public function admin_print_footer_scripts() {
			$this->editor( '', 'show-hide-tinymce-widget', 'show-hide-tinymce-widget' );
		}
		...
		public function wp_tiny_mce_init() {
			$script = 'show-hide-tinymce-widget-setup';
			$suffix = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
		...
		public function editor_settings( $settings, $editor_id ) {
			if ( strstr( $editor_id, 'show-hide-tinymce' ) ) {
				$settings['tinymce'] = array(
					'wp_skip_init' => 'widget-show-hide-tinymce-__i__-text' == $editor_id,
					'add_unload_trigger' => false,
					'wp_autoresize_on' => false,
				);
				$settings['editor_height'] = 350;
				$settings['dfw'] = true;
				$settings['editor_class'] = 'show-hide-tinymce';
			}
			return $settings;
		}
		...
		public function init_links() {
			$this->links = array(
				...
				'https://wordpress.org/plugins/show-hide-tinymce-widget/' => __( 'Home', 'show-hide-tinymce-widget' ),
				/* translators: text used for support faq link */
				'https://wordpress.org/plugins/show-hide-tinymce-widget/faq/' => __( 'FAQ', 'show-hide-tinymce-widget' ),
				/* translators: text used for support forum link */
				'https://wordpress.org/support/plugin/show-hide-tinymce-widget' => __( 'Support', 'show-hide-tinymce-widget' ),
				/* translators: text used for reviews link */
				'https://wordpress.org/support/view/plugin-reviews/show-hide-tinymce-widget' => __( 'Rate', 'show-hide-tinymce-widget' ),
				/* translators: text used for follow on twitter link */
				'https://twitter.com/blackstudioita' => __( 'Follow', 'show-hide-tinymce-widget' ),
				/* translators: text used for donation link */
				'http://www.blackstudio.it/en/wordpress-plugins/show-hide-tinymce-widget/' => __( 'Donate', 'show-hide-tinymce-widget' ),
			);
		}
...
//			echo "\t\t<a class='bstw-links-icon icon16 icon-plugins' href='#' title='" . esc_attr( __( 'About Black Studio TinyMCE Widget plugin', 'show-hide-tinymce-widget' ) ) . "'></a>\n";
//			echo "\t</div>\n";
//		}
	...
	} // END class Show_Hide_TinyMCE_Admin
