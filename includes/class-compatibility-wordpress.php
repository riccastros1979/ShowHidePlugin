...
if ( ! class_exists( 'Show_Hide_TinyMCE_Compatibility_Wordpress' ) ) {

	final class Show_Hide_TinyMCE_Compatibility_Wordpress {
		...
		public function wp_pre_33() {
			...
			if ( bstw()->admin()->enabled() ) {
				...
				add_filter( 'show-hide-tinymce-widget-script', array( $this, 'wp_pre_33_handle' ), 67 );
				add_filter( 'show-hide-tinymce-widget-style', array( $this, 'wp_pre_33_handle' ), 67 );
				...
				remove_filter( 'Show_Hide_tinymce_admin_pointers-widgets', array( bstw()->admin(), 'pointer_register' ) );
			}
		}
		 ...
		 * @uses Show_Hide_TinyMCE_Admin::enqueue_style()
		 *
		 * @return void
		 * @since 2.0.0
		 */
		 ...
		 * @uses Show_Hide_TinyMCE_Admin::enqueue_script()
		 * @uses Show_Hide_TinyMCE_Admin::localize_script()
		 *
		 * @return void
		 * @since 2.0.0
		 */
		...
		public function wp_pre_33_handle() {
			return 'show-hide-tinymce-widget-pre33';
		}
		 ...
		 *
		 * @uses add_filter()
		 * @uses Show_Hide_TinyMCE_Admin::enabled()
		 *
		 * @return void
		 * @since 2.0.0
		 */
		...
		public function wp_pre_35_upload_iframe_src( $upload_iframe_src ) {
			global $pagenow;
			if ( 'widgets.php' == $pagenow || ( 'admin-ajax.php' == $pagenow && isset( $_POST['id_base'] ) && 'show-hide-tinymce' == $_POST['id_base'] ) ) {
		...
		public function wp_pre_39() {
			$wp_version = get_bloginfo( 'version' );
			if ( bstw()->admin()->enabled() ) {
				add_filter( 'show-hide-tinymce-widget-script', array( $this, 'wp_pre_39_handle' ), 61 );
				...
				add_action( 'Show_Hide_tinymce_editor', array( $this, 'wp_pre_39_editor' ), 10, 4 );
				remove_action( 'Show_Hide_tinymce_editor', array( bstw()->admin(), 'editor' ), 10, 3 );
			}
		}
		...
		public function wp_pre_39_handle() {
			return 'show-hide-tinymce-widget-pre39';
		}
		...
		public function wp_pre_39_admin_print_footer_scripts() {
			if ( function_exists( 'wp_editor' ) ) {
				wp_editor( '', 'show-hide-tinymce-widget' );
			}
		}
	...
	} // END class Show_Hide_TinyMCE_Compatibility_Wordpress
