...
(function( $ ) {
	...
	bstw = function( arg ) {
		...
		// jQuery object initialization
			...
			id = $( 'textarea[id^=widget-show-hide-tinymce][id$=text]', arg.closest( bstw_data.container_selectors ) ).attr( 'id' );
		}
		...
		return {
			...
			activate: function ( force_init ) {
				...
				if ( ! $( '#' + id ).hasClass( 'active' ) ) {
					...
					if ( ! this.is_quicktags_configured() ) {
						tinyMCEPreInit.qtInit[ id ] = tinyMCEPreInit.qtInit['show-hide-tinymce-widget'];
					...
					if ( ! this.is_tinymce_configured() ) {
						tinyMCEPreInit.mceInit[ id ] = tinyMCEPreInit.mceInit['show-hide-tinymce-widget'];
			...
			get_mode: function() {
				return  $( 'input[id^=widget-show-hide-tinymce][id$=type]', this.get_container() ).val();
			},

			// Set editor mode ( visual / html ) into the input value
			set_mode: function( mode ) {
				$( 'input[id^=widget-show-hide-tinymce][id$=type]', this.get_container() ).val( mode );
				return this;
			},
	...
	$( document ).ready(function() {
		...
		$( document ).on( 'click', 'div.widget[id*=show-hide-tinymce] .widget-title, div.widget[id*=show-hide-tinymce] .widget-title-action', function() {
		...
		$( document ).on( 'widget-added', function( event, $widget ) {
			if ( $widget.is( '[id*=show-hide-tinymce]' ) ) {
		...
		$( document ).on( 'click', '[id^=widget-show-hide-tinymce][id$=tmce]', function() {
			bstw( $( this ) ).set_mode( 'visual' );
		});

		// Event handler for html switch button
		$( document ).on( 'click', '[id^=widget-show-hide-tinymce][id$=html]', function() {
		...
		$( document ).on( 'sortstart',  function( event, ui ) {
			...
			var open_widgets_selectors = [
				'body.wp-customizer .expanded > div[id*=show-hide-tinymce].widget', // Theme Customizer
				'.widget-liquid-right div[id*=show-hide-tinymce].widget.open' // Widgets page
			];
			...
			if ( ui.item.is( '[id*=show-hide-tinymce]' ) ){
				bstw( ui.item.find( 'textarea[id^=widget-show-hide-tinymce]' ) ).deactivate();
			}
		});
		$( document ).on( 'sortupdate',  function( event, ui ) {
			...
			$( 'body' ).addClass( 'wait' );
			setTimeout(function() {
				$( 'textarea[id^=widget-show-hide-tinymce].active' ).each(function() {
					bstw( $( this ) ).deactivate();
				});
		...
		$( document ).on( 'click', 'body.wp-customizer div[id*=show-hide-tinymce].widget .move-widget-btn', function(){
			$( 'body' ).addClass( 'wait' );
			var $btn = $( this );
			setTimeout(function() {
				$( 'textarea[id^=widget-show-hide-tinymce].active' ).each(function() {
		...
		$( window ).resize(function() {
			$( 'textarea[id^=widget-show-hide-tinymce]' ).each(function() {
				bstw( $( this ) ).responsive();
			});
		});

		// Deactivate quicktags toolbar on hidden base instance
		$( '#qt_widget-show-hide-tinymce-__i__-text_toolbar' ).remove();
