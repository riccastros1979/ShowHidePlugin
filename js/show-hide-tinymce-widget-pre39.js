...
(function( $ ) {
	...
	function bstw( arg ) {
		...
		// jQuery object initialization
			...
			id = $( 'textarea[id^=widget-show-hide-tinymce]', arg.closest( bstw_data.container_selectors ) ).attr( 'id' );
		}
		...
		return {
			...
			activate: function () {
				...
				if ( typeof tinymce === 'object' && typeof tinymce.execCommand === 'function' ) {
					...
					try {
						...
						tinymce.init( tinymce.extend( {}, tinyMCEPreInit.mceInit['show-hide-tinymce-widget'], tinyMCEPreInit.mceInit[id] ) );
			...
			get_mode: function() {
				return  $( 'input[id^=widget-show-hide-tinymce][id$=type]', this.get_widget_inside() ).val();
			...
			set_mode: function( value ) {
				...
				$( 'input[id^=widget-show-hide-tinymce][id$=type]', this.get_widget_inside() ).val( value );
				return this;
			},
	...
	$( document ).ready(function() {
		...
		$( document ).on( 'click', 'div.widget[id*=show-hide-tinymce] .widget-title, div.widget[id*=show-hide-tinymce] a.widget-action', function() {
		...
		$( 'div.widget[id*=show-hide-tinymce] input[name=savewidget]' ).on( 'click', function() {
			if ( bstw( $( this ) ).is_tinymce_active() ) {
				bstw( $( this ) ).deactivate();
			}
			// Event handler for ajax complete
		...
		$( document ).on( 'click', 'a[id^=widget-show-hide-tinymce][id$=tmce]', function() {
			bstw( $( this ) ).set_mode( 'visual' );
		});

		// Event handler for html switch button
		$( document ).on( 'click', 'a[id^=widget-show-hide-tinymce][id$=html]', function() {
		...
		if ( $( 'body.widgets_access' ).size() > 0 ) {
			bstw( $( 'textarea[id^=widget-show-hide-tinymce]' ).attr( 'id' ) ).activate_after_open();
		}
